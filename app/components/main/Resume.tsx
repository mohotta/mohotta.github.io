        ...
        <Slide duration={1000} direction='right'>
          ...
          <div className='w-full px-4 sm:px-6 lg:px-8 mt-16 sm:mt-20 lg:mt-24'>
            <div className='max-w-7xl mx-auto'>
              ...
              {
                experienceConfig.workExperience.map(company => (
                  <Fade key={company.id} duration={1000} className='flex justify-center items-stretch w-full sm:w-80 md:w-72 lg:w-80'>
                    <div className='flex flex-col justify-start items-center p-6 bg-card/50 backdrop-blur-sm border border-border/50 rounded-xl w-full hover:bg-card/70 transition-all duration-300 hover:scale-105'>
                      <h1 className='w-full font-bold text-lg text-foreground mb-2 text-center'> {company.company} </h1>
                      <p className='w-full text-sm text-muted-foreground text-right mb-4'> {company.location} </p>
                      {
                        company.positions.map(position => (
                          <div key={position.title} className='w-full'>
                            <hr className='my-2 border-border/30'/>
                            <p className='text-base font-semibold text-foreground'> {position.title} </p>
                            <p className='text-sm text-muted-foreground'> {position.startDate} - {position.endDate} </p>
                            <p className='text-xs text-muted-foreground mt-1 italic'> {position.type} </p>
                          </div>
                        ))
                      }
                    </div>
